...
const PersonsController = {
    index: async (req, res) => {
        ...
        if (nome) filters.name = new RegExp(nome, 'i');
    ...
    store: async (req, res) => {
        ...
        if (exist) {
            return res.status(400).json({ error: 'Cliente ou cpf já cadastrado.' });
        }
    ...
    update: async (req, res) => {
        const person = await Persons.findByIdAndUpdate(req.params.id, req.body, {
            new: true
        });
